...
const repositorioFake: IRepositorioTurmas = {
  ...
  async buscarMatriculasDaDisciplina() {
    return [
      ...
      { alunoId: "1", alunoNome: "Ana", notaP1: 7, notaP2: 9, faltas: 6, status: "ativo" as const, cargaHoraria: 60 }
    ];
  }
};

describe("GET /api/disciplinas/:disciplinaId/alunos", () => {
  it("retorna lista ordenada alfabeticamente", async () => {
    ...
    expect(response.body.alunos[1].nome).toBe("Bruno");
  });
});
